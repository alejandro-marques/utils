...
public class Processor {
    ...
    public Map<String, Object> nextDocument () throws Exception {
        ...
        try {document.putAll(generateDocument(isVariation, document));}
        catch (Exception exception){
    ...
    private Map<String, FieldValue> generateDocument (boolean isVariation,
            Map<String, FieldValue> document) throws Exception {
        return generateFields(model, isVariation, document);
    }
    ...
    private Map<String, FieldValue> generateId (List<FieldInfo> idFieldsInfo,
        ...
        if (null == lastId){return generateFields(idFieldsInfo, false, document);}
        ...
        while (currentLevel >= 0){
            try{
                ...
                id.put(currentFieldInfo.getName(),
                        ValueUtils.getValue(currentFieldInfo.getValue(), lastValue, id));
                ...
                for (int level = currentLevel + 1; level < idFieldsInfo.size() ; level++) {
                    ...
                    id.put(field.getName(), ValueUtils.getValue(field.getValue(), null, id));
                }
    ...
    private Map<String, FieldValue> generateFields (List<FieldInfo> fieldsInfo, boolean isVariation,
            Map<String, FieldValue> document) throws Exception {
        if (null != fieldsInfo) {
            for (FieldInfo fieldInfo : fieldsInfo) {
                String fieldName = fieldInfo.getName();
                try {
                    FieldValue lastValue = isVariation && null != lastDocument ?
                            lastDocument.get(fieldName) : null;
                    document.put(fieldName, ValueUtils.getValue(fieldInfo.getValue(), lastValue, document));
                }
                catch (Exception exception){
